    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      if (!type) {
        ...
        if (parts[0] === 'splat' && parts[1]) {
          ...
          type = 'glb';
          model = parts[0]; // /:slug â†’ GLB por defecto
        }
      }
      ...
      (async function init() {
        try {
          if (type === 'glb' && model) {
              ...
              onStall: () => {
                ...
                showErrorAndRetry('glb', model, () => {
                  ...
                  startProgressWatchdog(mv, {
                    timeoutMs: 9000,
                    onStall: () => showToast('TodavÃ­a no hay datos. Reintenta en unos segundos.', 2200)
                  });

                  // Re-mostrar pÃ³ster y skeleton durante el reintento
            ...
            // Deshabilita AR hasta que el modelo estÃ© listo
            setArEnabled(false);

            // FunciÃ³n de reintento (con cache-buster) y rehacer pÃ³ster overlay si existe
            const retryGlb = () => {
              const sk = document.getElementById('skel');
              if (sk) {
            ...
            // Auto-reintentar una vez al recuperar conexiÃ³n si el estado era de error
            window.addEventListener('online', () => {
              const msg = document.getElementById('msg');
              if (msg && !msg.classList.contains('hidden')) retryGlb();
            });

            // â–¼â–¼ AÃ‘ADE ESTA LÃNEA AQUÃ â–¼â–¼
            mv.classList.remove('hidden');

            // Failsafe: oculta el skeleton si algÃºn navegador no dispara 'load'
            setTimeout(() => {
              const sk = document.getElementById('skel');
            ...
            if (mv) {
              ...
              // â–¶ PÃ“STER GLB (overlay separado para poder hacer fade)
              ...
              // âœ… Detectar si es escritorio o mÃ³vil
              const isDesktop = window.matchMedia('(min-width: 1024px)').matches;
              const folder = isDesktop ? 'desktop' : 'mobile';

              // âœ… Crear URL adaptativa (con cache-buster)
              ...
              // âœ… Aplicar el poster como fondo del overlay
              ...
              console.log('âœ… Poster adaptativo cargado:', glbPosterUrl);

              // â–¶ Listener del load del model-viewer
              mv.addEventListener('load', () => {
                // Espera un frame de render para asegurar que el modelo ya estÃ¡ visible
              ...
              mv.addEventListener('load', () => {
                // ... tu cÃ³digo para ocultar skeleton ...

                // ya haces visible el modelo:
                requestAnimationFrame(() => mv.classList.add('is-visible'));

                // â–¶ Crossfade: poster GLB se desvanece tras un frame
                if (glbPoster) {
                  // da un pequeÃ±o margen para que el modelo ya estÃ© pintado
                  setTimeout(() => {
                    glbPoster.classList.add('fade-out');
                    // limpia el DOM tras la animaciÃ³n
                    setTimeout(() => {
                      glbPoster.remove();
                    }, 500); // â‰ˆ a tu transition .45s
                  }, 30);
                }
                // Modelo listo â†’ habilita AR
                setArEnabled(true);
              });
            }
            ...
            // Aplica solo iluminaciÃ³n (environment-image). Elimina cualquier skybox.
            ...
            // âœ… BotÃ³n AR unificado (optimizado y reseteado por carga)
            if (arBtn) {
              // Elimina handlers previos si la pÃ¡gina fue recargada desde memoria
              ...
              newArBtn.addEventListener('click', async () => {
                // âœ… VERIFICAR COOLDOWN: No permitir clicks mÃºltiples
                const now = Date.now();
                ...
                if (timeSinceLastClick < AR_COOLDOWN_MS) {
                  ...
                  // VibraciÃ³n de feedback negativo
                  try {
                ...
                // Actualizar timestamp del Ãºltimo click
                lastARClickTime = now;
            ...
            // Auto-open si el usuario lo dejÃ³ abierto para este slug o si viene con ?info=1 o #info
            const wantsInfo =
            ...
            if (wantsInfo || lastState === 'open') {
              // Deja un frame para que el DOM pinte antes del open (suaviza la transiciÃ³n)
          ...
          if (type === 'splat' && figure) {
            ...
            // Deshabilita AR mientras el SPLAT no estÃ© listo
            setArEnabled(false);
            ...
            // Si vuelve la conexiÃ³n y aÃºn no cargÃ³, reintenta
            window.addEventListener('online', () => {
            ...
            if (splatPoster) {
              ...
              splat.addEventListener(
                ...
                () => {
                  ...
                  // SPLAT ya pintado â†’ habilita AR y muestra la escena
                  setArEnabled(true);
                  requestAnimationFrame(() => splat.classList.add('is-visible'));

                  // Oculta skeleton por si seguÃ­a visible
                  const sk = document.getElementById('skel');
                  if (sk) {
                    sk.style.opacity = '0';
                    setTimeout(() => sk.classList.add('hidden'), 400);
        ...
        // ...otras funciones aquÃ­, como setupAudioFor(), openARFor(), etc.

        // ==== CARTELA DINÃMICA (versiÃ³n escritorio y mÃ³vil) ====
        async function loadInfoData() {
          try {
            ...
            if (!data[slug]) {
              console.warn(`No se encontrÃ³ informaciÃ³n para ${slug}`);
              return;
            }
            ...
            // Inserta la informaciÃ³n en la cartela
            infoPanel.innerHTML = `
            <h2>${info.title}</h2>
            <div class="info-meta">${info.author} Â· ${info.year}</div>
            <div class="info-desc">${info.desc}</div>

            <ul class="info-details">
            ${info.material ? `<li><strong>Material:</strong> ${info.material}</li>` : ""}
            ${(info.tipologia || info.typology) ? `<li><strong>TipologÃ­a:</strong> ${info.tipologia || info.typology}</li>` : ""}
            ${(info.tecnica || info.technique) ? `<li><strong>TÃ©cnica:</strong> ${info.tecnica || info.technique}</li>` : ""}
            ...
            <div class="info-actions">
              ...
              <a href="https://museovirtualsantana.es" target="_blank" rel="noopener" class="info-btn-visit">
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          ...
          `;
            ...
            async function preloadAudio(slug) {
              try {
                ...
                audio.preload = "auto";
                console.log(`ðŸŽ§ Precargado: ${audioPath}`);
              } catch (err) {
        ...
        // ðŸ‘‡ðŸ‘‡ MantÃ©n esto abajo del todo, antes del cierre:
      })();

    })();

    /* === PANEL VISUAL DE AJUSTE DE CÃMARA (con min/max zoom) === */
    (function setupCameraDebugPanel() {
      ...
      panel.style.cssText = `
        ...
        `;
      panel.innerHTML = `
          <b>ðŸŽ¥ Ajuste de CÃ¡mara</b><br><br>
          ...
          <label>ElevaciÃ³n (phi): <span id="valPhi">${initPhi.toFixed(0)}</span>Â°</label><br>
          <input id="ctrlPhi" type="range" min="0" max="90" step="1" value="${initPhi}"><br><br>

          <label>FOV (zoom Ã³ptico): <span id="valFov">${initFov.toFixed(0)}</span>Â°</label><br>
          <input id="ctrlFov" type="range" min="15" max="75" step="1" value="${initFov}"><br><br>

          <hr style="border-color:#2a2f38; opacity:.4; margin:8px 0">

          <label>Zoom mÃ­nimo (minRadius): <span id="valMinR">${initMinR.toFixed(1)}</span> m</label><br>
          <input id="ctrlMinR" type="range" min="0.4" max="60" step="0.1" value="${initMinR}"><br><br>

          <label>Zoom mÃ¡ximo (maxRadius): <span id="valMaxR">${initMaxR.toFixed(1)}</span> m</label><br>
...
"${model || 'cristo-yacente'}": {
  ...
  maxFov: "${fovRange.value}deg"
        ...
        setTimeout(() => (btn.textContent = 'ðŸ“‹ Copiar valores'), 1200);
      };
    })();

    /* === InteracciÃ³n tÃ¡ctil para arrastrar la cartela (UX mejorado + fix reopen) === */
    (function enableInfoSheetDrag() {
      ...
      const threshold = 90; // pÃ­xeles de arrastre necesarios para cerrar
      ...
      const closeSheetSmooth = () => {
        ...
        // pequeÃ±o retraso para permitir la animaciÃ³n antes del hidden
        setTimeout(() => {
          sheet.classList.add('hidden');
          sheet.setAttribute('aria-hidden', 'true');
    ...
    /* ðŸ”’ Bloquear page-zoom del navegador solo cuando el gesto ocurre sobre la UI */
    (function preventPageZoomOnUI() {
      ...
      /* iOS: gesturestart/gesturechange â†’ pinch page-zoom */
      const onGesture = (e) => {
        if (isInUI(e.target)) {
      ...
      /* Trackpad pinch (Ctrl+wheel) en Desktop â†’ evita zoom si es sobre UI */
      ...
      const onTouchMove = (e) => {
        if (isInUI(e.target)) {
          // permitimos desplazamiento en la cartela, pero no â€œpinch zoomâ€ de pÃ¡gina
      ...
      /* === Carga de pÃ³sters adaptativos (desktop vs mÃ³vil) === */
      (function loadAdaptivePoster() {
        ...
        // Quitar el pÃ³ster cuando el viewer cargue
        const mv = document.getElementById('mv');
        const splat = document.getElementById('splat');
      ...
      // ==== SPLAT POSTER (aÃ±adido manualmente) ====
      const splatPoster = document.getElementById('splatPoster');
  ...
  <script>
    document.addEventListener('DOMContentLoaded', () => {
      ...
      // --- FunciÃ³n unificada de carga de informaciÃ³n ---
      function loadInfoForSlug(slug) {
        const infoPath = `/assets/info.json`;
        fetch(infoPath)
          .then(res => res.json())
          .then(infoData => {
            const info = infoData[slug];
          ...
          `;
            ...
            console.log(`[Museo] Datos cargados de ${infoPath}`);
          })
          .catch(err => console.error('[Museo] Error cargando info.json:', err));
      }

      // Cargar idioma inicial al abrir la pÃ¡gina
      loadInfoForSlug(slug);
    });
  </script>
  <script>
